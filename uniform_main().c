...
smaple points t is 0,1,2,3,4,5,6,7,8
*/
...
int main(){
	...
	printf("\n");
	//printf("%lf", Newton_interpolate(X, 9.0, N));
	printf("\n");
	/* Y */
	for (i = 0; i <= N; i++) {
		Y[i][0] = i;
	...
	FILE* fp;
	fp = fopen("Q3A_uniform_X1.txt", "w");
	for (i = 0; i < 39; i++) {
		fprintf(fp, "%lf;", points[i][0]);
	}
	fprintf(fp, "%lf;", points[i][0]);

	fp = fopen("Q3A_uniform_Y1.txt", "w");
	for (i = 0; i < 39; i++) {
	...
	//printf("%lf", Newton_interpolate(Y, 9.0, N));
	return 0;
}